...
<div class="withdrawals-form">
    ...
    <?php 
        ...
        echo $model->is_test ? '<span class="fs-5 fw-bold text-danger">Тестовая заявка</span>' : '';
        echo $form->errorSummary($model);
    ?>

    <?= $from === 'create' ? (isset($_GET['id']) && $_GET['id'] > 0 ? $form->field($model, 'client_id')->dropDownList([$_GET['id'] => app\models\Clients::find()->where(['id' => $_GET['id']])->one()->shop], ['class' => 'form-control', 'style' => 'cursor: pointer;', 'disabled' => true])->label('Клиент') : $form->field($model, 'client_id')->dropDownList(yii\helpers\ArrayHelper::map(app\models\Clients::find()->all(), 'id', 'shop'), ['class' => 'form-control', 'style' => 'cursor: pointer;', 'disabled' => true])->label('Клиент')) : ''; ?>
